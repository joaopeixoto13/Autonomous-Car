...
def filterExtreme(points):
    ...
    for i in range(0,len(points)):
        if(points[i][0] > x_min and points[i][0] < x_max and points[i][1] > y_min and points[i][1] < y_max):    # If the point is inside the filter
...
def varianceAlgorithm(img):
    ...
    if len(right) > 0 and len(left) > 0:                                        # If there are contours
        u = control(filterExtreme(left),filterExtreme(right))                   # Get the control signal
    else:                                                                       # If there are no contours
        return 0
        #print("No Line --> In Front")                                           # Print that there is no line
        #u = u_prev                                                              # Set the control signal to the previous control signal
    #cv.imshow("img",img)
